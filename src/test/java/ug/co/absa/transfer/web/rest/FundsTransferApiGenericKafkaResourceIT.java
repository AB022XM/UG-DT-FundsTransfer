...
@WithMockUser
@EmbeddedKafka
class FundsTransferApiGenericKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/funds-transfer-api-generic-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/funds-transfer-api-generic-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/funds-transfer-api-generic-kafka/unregister"));
                return;
            }
        }
